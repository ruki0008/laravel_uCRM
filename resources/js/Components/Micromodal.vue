...
onMounted(async () => {
  try {
    await axios.get('/sanctum/csrf-cookie')
    const res = await axios.get('/api/user')
    console.log('ログイン済み:', res.data)
    user.value = res.data
  } catch (err) {
    console.log('未ログイン or エラー', err)
  }
})
...
<template>
    ...
    <button @click="searchCustomers" type="button" data-micromodal-trigger="modal-1" class="flex mx-auto mt-2 text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg">検索する</button>
</template>